...
typedef NS_ENUM(NSUInteger, StoreValueType) {
    ...
    kStoreValueType_Collection,     //  JSON, NSArray, NSDictionary ...
    kStoreValueType_Data,           //  NSData => base64
    kStoreValueType_Error,          //  非法数据
};

@interface JMKeyValueItem : NSObject
...
- (id)valueForKey:(NSString *)key fromTable:(NSString *)tableName {
    JMKeyValueItem * item = [self itemForKey:key fromTable:tableName];
    if (item) {
        return item.value;
    } else {
        return nil;
...
- (JMKeyValueItem *)itemForKey:(NSString *)key fromTable:(NSString *)tableName {
    ...
    if (content) {
        ...
        JMKeyValueItem * item = [[JMKeyValueItem alloc] init];
        item.key = key;
        item.value = result;
...
- (NSArray *)allItemsFromTable:(NSString *)tableName {
    ...
    NSError * error;
    for (JMKeyValueItem * item in result) {
        error = nil;
